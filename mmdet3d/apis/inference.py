...
def ReadCalibFile(CalibrationFile):
    ...
    calibdata = read_calib_file(CalibrationFile)
    calibdata['R0_rect'] = np.reshape(calibdata['R0_rect'], (4, 4)).astype(np.float32)
    calibdata['Tr_velo_to_cam'] = np.reshape(calibdata['Tr_velo_to_cam'], (4, 4)).astype(np.float32)
...
def inference_multi_modality_detector(model, pcd, image, ann_file):
    ...
    # if configuration is changed, you need to modify here for your hardware setup.
    data_infos = ReadCalibFile(ann_file)
    rect = data_infos['R0_rect'].astype(np.float32)
    Trv2c = data_infos['Tr_velo_to_cam'].astype(np.float32)
    P2 = data_infos['P2'].astype(np.float32)
    ...
    sensor2lidar = (np.linalg.inv(intrinsic_4x4) @ lidar2img).T
    caminfos.append({
        'sensor2lidar_rotation': sensor2lidar[:3, :3].T,
    ...
    data = dict(
        pts_file=pcd,
        img_file=image,
        img_info=dict(filename=img_name, intrinsic_4x4=intrinsic_4x4),
        lidar2img=lidar2img,        # For PointFusion
        caminfo=caminfos,
        box_type_3d=box_type_3d,
        box_mode_3d=box_mode_3d,
        img_fields=[],
    ...
    # Depth to image conversion
        ...
        rt_mat = data_infos[0]['calib']['Rt']
        # follow Coord3DMode.convert_point
        rt_mat = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]
                           ]) @ rt_mat.transpose(1, 0)
        depth2img = data_infos[0]['calib']['K'] @ rt_mat
...
def show_det_result_meshlab(data,
    ...
    pts_file = data['img_metas'][0][0]['filename']
    file_name = osp.split(pts_file)[-1].split('.')[0]
...
def show_seg_result_meshlab(data,
    ...
    points = data['points'][0][0].cpu().numpy()
    pts_file = data['img_metas'][0][0]['pts_file']
    file_name = osp.split(pts_file)[-1].split('.')[0]
