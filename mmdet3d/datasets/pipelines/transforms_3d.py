...
class RandomFlip3D(RandomFlip):
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        if 'lidar2img' in input_dict:
            # HACK(soonminh)
            w = input_dict['ori_shape'][1]
            intrinsic_4x4 = input_dict['img_info']['intrinsic_4x4']
            lidar2img_tr = np.linalg.inv(intrinsic_4x4) @ input_dict['lidar2img']
            intrinsic_4x4[0][2] = w - intrinsic_4x4[0][2]
            input_dict['lidar2img'] = intrinsic_4x4 @ lidar2img_tr

        if 'caminfo' in input_dict:
            # HACK(soonminh)
            w = input_dict['ori_shape'][1]
            for ii in range(len(input_dict['caminfo'])):
                if 'cam_intrinsic' in input_dict['caminfo'][ii]:
                    input_dict['caminfo'][ii]['cam_intrinsic'][0][2] = w - input_dict['caminfo'][ii]['cam_intrinsic'][0][2]
    ...
    def __call__(self, input_dict):
        ...
        # HACK(soonminh)
        # When using this with "LoadMultiViewImageFromFiles",
        # we need to change mmdet/datasets/pipelines/transforms.py:L467-469 to below:
        #     for key in results.get('img_fields', ['img']):
        #         # HACK(soonminh): can be multiple images
