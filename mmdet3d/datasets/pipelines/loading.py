    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if 'lidar2img' in results:
                results['lidar2img'][0, 0] *= w_scale
                results['lidar2img'][1, 1] *= h_scale

            if 'caminfo' in results:
                for ii in range(len(results['caminfo'])):
                    if 'cam_intrinsic' in results['caminfo'][ii]:
                        results['caminfo'][ii]['cam_intrinsic'][0, 0] *= w_scale
                        results['caminfo'][ii]['cam_intrinsic'][1, 1] *= h_scale
...
class LoadMultiViewImageFromFiles(object):
    ...
    Expects results['img_file'] to be a list of filenames.
    ...
    def __call__(self, results):
        ...
        # if img_file is string for imag path
        if isinstance(results['img_file'], str):
            filename = results['img_file']
            # img is of shape (h, w, c, num_views)
            if self.img_scale is None: # shape: 375, 1242, 3, 1
                img = np.stack([mmcv.imread(filename, self.color_type)], axis=-1)
            else:
                img = np.stack([self.pad(mmcv.imread(filename, self.color_type))], axis=-1)

            results['filename'] = filename
        else: # img file is raw sensor file, shape: H,W,C,N (number of view) 
            img = np.stack([results['img_file']], axis=-1)
...
class LoadPointsFromMultiSweeps(object):
    ...
    def _load_points(self, pts_file):
        """Private function to load point clouds data.

        Args:
            pts_file (str): Filename of point clouds data.
        ...
        try:
            pts_bytes = self.file_client.get(pts_file)
            points = np.frombuffer(pts_bytes, dtype=np.float32)
        except ConnectionError:
            mmcv.check_file_exist(pts_file)
            if pts_file.endswith('.npy'):
                points = np.load(pts_file)
            else:
                points = np.fromfile(pts_file, dtype=np.float32)
        return points
...
class LoadPointsFromFile(object):
    ...
    def __call__(self, results):
        ...
        """
        if isinstance(results['pts_file'], str) :
            pts_file = results['pts_file']
            points = self._load_points(pts_file)
        else:
            points = results['pts_file']
