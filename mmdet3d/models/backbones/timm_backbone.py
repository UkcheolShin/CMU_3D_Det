...
def print_timm_feature_info(feature_info):
    ...
    if feature_info is None:
        warnings.warn('This backbone does not have feature_info')
    elif isinstance(feature_info, list):
        for feat_idx, each_info in enumerate(feature_info):
            warnings.warn(f'backbone feature_info[{feat_idx}]: {each_info}')
    else:
        try:
            warnings.warn(f'backbone out_indices: {feature_info.out_indices}')
            warnings.warn(f'backbone out_channels: {feature_info.channels()}')
            warnings.warn(f'backbone out_strides: {feature_info.reduction()}')
        except AttributeError:
            warnings.warn('Unexpected format of backbone feature_info')
