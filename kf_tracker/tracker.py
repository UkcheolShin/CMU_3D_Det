...
from kf_tracker.bbox_utils import convert_3dbox_to_8corner, iou3d
from kf_tracker.kalman_filter import KalmanBoxTracker
...
class AB3DMOT(object):			  # A baseline of 3D multi-object tracking
	def __init__(self, max_age=5, min_hits=3):      # max age will preserve the bbox does not appear no more than 2 frames, interpolate the detection
		"""
		Sets key parameters for SORT                
        observation: [x, y, z, rot_y, l, w, h]
            [x] before reorder: [x, y, z, l, w, h, rot_y]
            [x] after reorder:  [x, y, z, rot_y, l, w, h]
        state:
	...
	def update(self, dets_all):
		...
		"""
		dets, confs, labels = dets_all['dets'], dets_all['confs'], dets_all['labels']         # dets: N x 7, float numpy array
		...
		for t, trk in enumerate(self.trackers):
			if t not in unmatched_trks:
				...
				trk.update(dets[d, :][0], confs[d], labels[d])

		# create and initialise new trackers for unmatched detections
		for i in unmatched_dets:        # a scalar of index
			trk = KalmanBoxTracker(dets[i, :], confs[i], labels[i]) 
			self.trackers.append(trk)

		i = len(self.trackers)
		for trk in reversed(self.trackers):
			d = trk.get_state()      # bbox location
			d = d[self.reorder_back]			# change format from [x,y,z,theta,l,w,h] to [x,y,z,theta,l,w,h]

			if ((trk.time_since_update < self.max_age) and (trk.hits >= self.min_hits or self.frame_count <= self.min_hits)):      
				ret.append(np.concatenate((d, [trk.id + 1], trk.conf, trk.label)).reshape(1, -1)) # +1 as MOT benchmark requires positive
			i -= 1
